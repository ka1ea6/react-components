...
const [tasks, setTasks] = useState<Task[]>(initialTasks);
  ...
  const [currentView, setCurrentView] = useState<'kanban' | 'planning' | 'knowledge' | 'files' | 'epics'>('kanban');
  ...
  const handleViewChange = (view: 'kanban' | 'planning' | 'files' | 'epics') => {
  ...
  const onViewChange = (view: 'kanban' | 'planning' | 'files' | 'epics') => {
    // Hide planning view on mobile
    if (view === 'planning' && window.innerWidth < 768) {
      return;
    }
