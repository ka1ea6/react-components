...
}> = ({ colleagues, onAddReminder, trigger }) => {
              ...
              <div className="grid grid-cols-2 gap-4 pl-6">
                <div>
                  ...
                  <Select 
                    ...
                    onValueChange={(value) => 
                      setFormData({ ...formData, recurrencePattern: value as 'daily' | 'weekly' | 'monthly' | 'yearly' })
                    }
                  >
                    <SelectTrigger>
                      <SelectValue />
...
const EditReminderModal: React.FC<{
  ...
  const [formData, setFormData] = useState({
    ...
    recurrencePattern: reminder.recurrencePattern as 'daily' | 'weekly' | 'monthly' | 'yearly' || 'weekly',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onUpdateReminder(reminder.id, updates);
    onClose();
  };
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center space-x-4 text-sm text-muted-foreground">
                <div className="flex items-center space-x-1">
                  ...
                  <span className={overdue && !reminder.isCompleted ? 'text-destructive font-medium' : ''}>
                    {getDateLabel()}
                  </span>
                </div>
                ...
                <div className="flex items-center space-x-1">
                  ...
                  <span>{reminder.colleague.name}</span>
                </div>
              </div>
...
export const TasksView: React.FC<TasksViewProps> = ({
  ...
  const [colleagues] = useState<DigitalColleague[]>(initialColleagues);
  ...
  const stats = {
    ...
    overdue: reminders.filter(r => !r.isCompleted && isOverdue(r.dueDate, r.dueTime)).length,
    today: reminders.filter(r => isToday(r.dueDate)).length
  };
          ...
          {sortedReminders.length === 0 ? (
            <Card className="p-8 text-center">
              <div className="text-muted-foreground">
                {searchTerm || filter !== 'all' ? 'No reminders match your filters' : 'No reminders yet'}
              </div>
