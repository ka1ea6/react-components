      ...
      >
        <div className="space-y-2">
          <div className="flex items-start justify-between">
            <h4 className="font-medium text-sm text-foreground line-clamp-1 select-none pointer-events-none">{task.title}</h4>
        ...
        <div className="w-full">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              ...
              <Badge variant="secondary" className="bg-muted text-muted-foreground">{backlogTasks.length}</Badge>
            </div>
            
            <div className={`space-y-3 overflow-y-auto max-h-[600px] min-h-[200px] border-2 border-dashed rounded p-3 transition-colors ${
          ...
          <div className="space-y-4">
            ...
            <Card className="p-4 bg-card shadow-sm">
              <div className="flex items-center justify-between mb-3">
                <h3 className="font-semibold text-foreground select-none">Select Sprints to View</h3>
                <Button
                  onClick={() => setIsAddSprintModalOpen(true)}
                ...
                .map(sprint => {
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-4">
                        <div>
                          <h3 className="font-semibold text-foreground select-none">{sprint!.name}</h3>
                          {sprint!.isActive && (
                            <span className="text-xs text-success font-medium select-none">Active Sprint</span>
                          )}
                        </div>
                        <Badge variant="secondary" className="bg-muted text-muted-foreground">{sprintTasksFiltered.length}</Badge>
                      </div>
                      
                      <div className={`space-y-3 overflow-y-auto max-h-[500px] min-h-[200px] border-2 border-dashed rounded p-3 transition-colors ${
                        dragOverTarget === sprint!.id ? 'border-primary bg-primary/10' : 'border-border'
                      }`}>
